...
def test_submit_returns_job_id(client, valid_key, sample_jpeg):
    ...
    assert resp.json()["job_id"] == "urology-job-123"
...
def test_query_returns_job_id(client, valid_key):
    with patch("app.routes._query.Queue") as mock_q:
        mock_job = MagicMock()
        mock_job.id = "urology-query-001"
        mock_q.return_value.enqueue.return_value = mock_job
        resp = client.post(
            "/v1/urology/query",
            json={"question": "What are common causes of kidney stones?"},
            headers={"X-API-Key": valid_key},
        )
    assert resp.status_code == 202
    assert resp.json()["job_id"] == "urology-query-001"
