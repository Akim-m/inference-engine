...
def test_submit_returns_job_id(client, valid_key, sample_jpeg):
    with patch("app.routes._analyze.Queue") as mock_q_cls:
        mock_job = MagicMock()
        mock_job.id = "rheum-job-123"
        mock_q_cls.return_value.enqueue.return_value = mock_job
        resp = client.post(
            "/v1/rheumatology/analyze",
            files={"image": ("joint.jpg", io.BytesIO(sample_jpeg), "image/jpeg")},
            headers={"X-API-Key": valid_key},
        )
    assert resp.status_code == 202
    assert resp.json()["job_id"] == "rheum-job-123"
...
def test_query_returns_job_id(client, valid_key):
    with patch("app.routes._query.Queue") as mock_q:
        mock_job = MagicMock()
        mock_job.id = "rheum-query-001"
        mock_q.return_value.enqueue.return_value = mock_job
        resp = client.post(
            "/v1/rheumatology/query",
            json={"question": "What joint changes are seen in rheumatoid arthritis?"},
            headers={"X-API-Key": valid_key},
        )
    assert resp.status_code == 202
    assert resp.json()["job_id"] == "rheum-query-001"
